(ns com.sixsq.slipstream.ssclj.resources.session-oidc.utils
  (:require
    ...
    [clj-http.client :as http]                              ;; FIXME: Should use kvlt for this.
...
(def ^:const authorize-query-params "?response_type=code&client_id=%s&redirect_uri=%s")


(def ^:const base-url-authorize-template (str "/auth" authorize-query-params))


(defn get-oidc-access-token-from-token-url
  [oidc-client-id oidc-token-url oidc-code redirect-uri]
  (-> (http/post oidc-token-url
...
(defn config-params
  ...
  (let [cfg-id (str "configuration/session-oidc-" instance)
    ...
    (try
      (let [{:keys [clientID publicKey baseURL tokenURL authorizeURL]} (crud/retrieve-by-id cfg-id opts)]
        (cond
          (and clientID publicKey baseURL) [clientID publicKey (str baseURL "/token") (str baseURL base-url-authorize-template)]
          (and clientID publicKey tokenURL authorizeURL) [clientID publicKey tokenURL (str authorizeURL authorize-query-params)]
          :else (throw-bad-client-config cfg-id redirectURI)))
      (catch Exception _
