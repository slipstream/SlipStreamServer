...
;;
;; atom to keep track of the loaded DeploymentTemplate resources
;;
(def templates (atom {}))
...
(defn register
  "Registers a given DeploymentTemplate resource and its description
   ...
   the resource as 'deployment-template/key'."
  [resource desc]
  (when-let [{:keys [id] :as full-resource} (complete-resource resource)]
    (swap! templates assoc id full-resource)
    (log/info "loaded DeploymentTemplate" id)
    (when desc
      (let [acl (:acl full-resource)
            full-desc (assoc desc :acl acl)]
        (swap! descriptions assoc id full-desc))
      (log/info "loaded DeploymentTemplate description" id))))

;;
;; schemas
;;

(def DeploymentTemplateDescription
  (merge c/CommonParameterDescription
         {:method {:displayName "Module reference URI"
                   :category    "general"
                   :description "module to be used for the creation of a deployment"
                   :type        "string"
                   :mandatory   true
                   :readOnly    true
                   :order       10}}))
...
(defmulti validate-subtype
          "Validates the given resource against the specific
           DeploymentTemplate subtype schema."
          :method)

(defmethod validate-subtype :default
  [resource]
  (throw (ex-info (str "unknown DeploymentTemplate type: " (:method resource)) resource)))
...
(defmethod crud/query resource-name
  ...
  (let [wrapper-fn (collection-wrapper-fn resource-name collection-acl collection-uri resource-tag)
        ...
        options (select-keys request [:identity :query-params :cimi-params :deployment-name :deployment-roles])
