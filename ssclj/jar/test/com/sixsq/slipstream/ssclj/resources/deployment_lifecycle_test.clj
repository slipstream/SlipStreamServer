...
(deftest create-deployment
  ...
  (let [href (str dt/resource-url "/" std/method)
    ...
    (let [create-req href-create
      ...
      ;; user should be able to see and start, terminate and delete deployment
      (-> session-user
          (request abs-uri)
          (ltu/body->edn)
          (ltu/is-status 200)
    ...
    (let [create-req href-create
      ...
      ;; admin should be able to see and start, terminate and delete deployment
      (-> session-admin
          (request abs-uri)
          (ltu/body->edn)
          (ltu/is-status 200)
  ...
  (let [href (str dt/resource-url "/" std/method)
    ...
    (let [create-req href-create
          ...
          start-uri (str p/service-context
                         (ltu/get-op deployment "http://schemas.dmtf.org/cimi/2/action/start"))
      ...
      (is (= dsm/initializing-state (get-in started-deployment [:response :body :state])))

      (are [expected value]
        (= expected value)
        dsm/initializing-state (uzk/get-data (str zdu/separator uri "/state"))
...
(deftest update-deployment-move-states-cancel
  ...
  (let [href (str dt/resource-url "/" std/method)
    ...
    (let [create-req href-create
          ...
          start-uri (str p/service-context
                         (ltu/get-op deployment "http://schemas.dmtf.org/cimi/2/action/start"))
          terminate-uri (str p/service-context
                             (ltu/get-op deployment "http://schemas.dmtf.org/cimi/2/action/terminate"))
          started-deployment (-> session-user
                                 (request start-uri :request-method :post)
                                 (ltu/body->edn)
                                 (ltu/is-status 200))
      ...
      (-> session-user
          (request terminate-uri :request-method :post)
          (ltu/body->edn)
          (ltu/is-status 200))

      (is (= dsm/cancelled-state (uzk/get-data (str zdu/separator uri "/state"))))


      (is (= dsm/cancelled-state (-> session-user
                                     (request (str p/service-context uri))
                                     (ltu/body->edn)
                                     (get-in [:response :body :state]))))
