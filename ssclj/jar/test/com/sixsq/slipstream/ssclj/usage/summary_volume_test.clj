...
(defn check-summaries
      ...
      (doseq [summary (summaries-from-db)]
             (if (= "exoscale-ch-gva" (:cloud summary))
               (do
                 (is (= "joe" (:user summary)))
                 (is (= {:disk-GB {:value       211050.0
                                   :aggregation "minutes"},
                         :RAM-GB  {:value       16800.0
                                   :aggregation "minutes"},
                         :nb-cpu  {:value       4200.0
                                   :aggregation "minutes"}} (:usage summary))))
               (do
                 (is (= "aws" (:cloud summary)))
                 (is (= "joe" (:user summary)))
                 (is (= {:disk-GB {:value       428400.0
                                   :aggregation "minutes"},
                         :RAM-GB  {:value       13440.0
                                   :aggregation "minutes"},
                         :nb-cpu  {:value       3360.0
                                   :aggregation "minutes"}} (:usage summary)))))))
...
(deftest check-precision-small-interval
         (let [start-day (t/date-time 2014)
              ...
              (let [summary (-> (summaries-from-db) first :usage)]
                   (is (= 83.75 (get-in summary [:disk-GB :value])))
                   (is (< 6.666 (get-in summary [:RAM-GB :value]) 6.667))
                   (is (< 1.666 (get-in summary [:nb-cpu :value]) 1.667)))))
...
(deftest multiple-users-same-cloud
         (let [start-day (t/date-time 2014)
              ...
              (let [summaries (summaries-from-db)]
                   (is (= [
                           {:disk-GB {:value       144720.0
                                      :aggregation "minutes"},
                            :RAM-GB  {:value       11520.0
                                      :aggregation "minutes"},
                            :nb-cpu  {:value       2880.0
                                      :aggregation "minutes"}}]
