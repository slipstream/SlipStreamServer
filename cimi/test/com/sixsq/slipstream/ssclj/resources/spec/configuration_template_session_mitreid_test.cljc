(ns com.sixsq.slipstream.ssclj.resources.spec.configuration-template-session-mitreid-test
  (:require
    ...
    [com.sixsq.slipstream.ssclj.resources.spec.configuration-template-session-mitreid :as cfg-tpl]
...
(deftest test-configuration-template-schema-check
  (let [timestamp "1964-08-25T10:00:00.0Z"
    ...
    (stu/is-valid ::cfg-tpl/session-mitreid root)

    (stu/is-invalid ::cfg-tpl/session-mitreid (assoc root :bad "BAD"))

    (doseq [k #{:id :resourceURI :created :updated :acl
                :service :instance
                :clientID :clientSecret :authorizeURL :tokenURL :userProfileURL :publicKey}]
      (stu/is-invalid ::cfg-tpl/session-mitreid (dissoc root k)))))
