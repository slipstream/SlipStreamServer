...
public class ConcurrentHashMapType<V, T> implements UserCollectionType {
	...
	public Object instantiate(int anticipatedSize) {
		return anticipatedSize <= 0 
		       ? new ConcurrentHashMap<V, T>()
		       : new ConcurrentHashMap<V, T>(anticipatedSize + (int)(anticipatedSize * .75f), .75f);
	}
	...
	public Object indexOf(Object collection, Object element) {
		Iterator<T> iter = ((ConcurrentHashMap)collection).entrySet().iterator();
		while (iter.hasNext()) {
	...
	public Object replaceElements(Object original, Object target,
		...
		ConcurrentHashMap result = new ConcurrentHashMap<V, T>();
		result.clear();
		Iterator<T> iter = ((ConcurrentHashMap)original).entrySet().iterator();
		while (iter.hasNext()) {
			ConcurrentHashMap.Entry me = (ConcurrentHashMap.Entry)iter.next();
