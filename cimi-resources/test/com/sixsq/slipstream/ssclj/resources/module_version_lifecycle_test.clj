(ns com.sixsq.slipstream.ssclj.resources.module-version-lifecycle-test
  (:require
...
(def valid-module-version {:id          "module/my-uuid-string"
...
(deftest lifecycle
  (let [session-anon (-> (ltu/ring-app)
    ...
    (-> session-anon
        (request base-uri
                 :request-method :post
                 :body (json/write-str valid-module-version))
        (ltu/body->edn)
        (ltu/is-status 403))

    ;; check module-version creation
    (let [admin-uri (-> session-admin
      ...
      ;; admin should see 2 module-version resources
      (-> session-admin
          (request base-uri)
          (ltu/body->edn)
          (ltu/is-status 200)
      ...
      ;; user should see only 1 module-version resource
      (-> session-user
          (request base-uri)
          (ltu/body->edn)
          (ltu/is-status 200)
      ...
      ;; verify contents of admin module-version
      ...
      ;; verify contents of user module-version
      ...
      ;; admin can delete the module-version
      (-> session-admin
          (request admin-abs-uri :request-method :delete)
          (ltu/body->edn)
          (ltu/is-status 200))

      ;; user can delete the module-version
      (-> session-user
