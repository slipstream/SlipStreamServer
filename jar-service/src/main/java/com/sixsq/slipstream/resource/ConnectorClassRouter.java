...
public class ConnectorClassRouter extends Router {

	public ConnectorClassRouter(Context context) throws ConfigurationException, ValidationException {
		super(context);
		...
		redirector = new ConnectorClassRedirector(getContext(), target, Redirector.MODE_SERVER_OUTBOUND);
		attach("/{conn}", redirector);

		// connector instance root
		redirector = new ConnectorClassRedirector(getContext(), hostname, Redirector.MODE_SERVER_OUTBOUND);
		attach("/", redirector);
	}
}
