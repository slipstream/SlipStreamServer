...
public class RegistrationResource extends SimpleRepresentationBaseResource {
	...
	final private static Document document;

	static {
		document = ResourceUtils.getResourceAsDocument(
				RegistrationResource.class, "register.xml");
	}

	@Get("html")
	public Representation toHtml() throws ResourceException {

		Document template = (Document) document.cloneNode(true);

		XmlUtil.addUser(template, user);
		XmlUtil.addBreadcrumbs(template, "", resourceUri);

		Source source = new DOMSource(template);

		return HtmlUtil.sourceToHtmlRepresentation(source, baseUrlSlash,
				resourceUri, configuration.version, "raw-content.xsl");
	}
	...
	public void createNewAccount(Representation entity)
		...
		setMessage(MessageUtils.format(MSG_REGISTRATION_SENT));
	...
	private void notifyUser(User newUser, OneShotAction action) {
		...
		boolean sendOk = Notifier
				.sendNotification(cfg, newUser.getEmail(), msg);
