...
public class DeploymentFactory extends RunFactory {
	...
	private static void checkAllImagesHaveReferenceOrImageId(Run run) throws ValidationException {
		...
		for (Node node : deployment.getNodes().values()) {
			...
			try {
				...
				throw new ValidationException("Node " + node.getName() + " refers to image " + ex.getMessage());
			}
		}
	}
	...
	private static void checkImageHasReferenceOrImageId(ImageModule image, String cloudServiceName)
		...
		if (mustHaveImageId && "".equals(image.getCloudImageId(cloudServiceName))) {
		...
		} else if (image.getModuleReferenceUri() == null || "".equals(image.getModuleReferenceUri())) {
			throw new ValidationException(image.getName() + " missing a machine image reference");

		} else {
			String referenceUri = image.getModuleReferenceUri();
			...
			if (reference == null) {
				throw new ValidationException(image.getName() + " referring to an unknown image "
						+ image.getModuleReferenceUri());
			}
			checkImageHasReferenceOrImageId(reference, cloudServiceName);
		}
	}

	private static void initNodesInstancesRuntimeParameters(Run run) throws ValidationException, NotFoundException {
		...
		for (Node node : deployment.getNodes().values()) {
			...
			run.addNode(node.getName(), run.getCloudServiceNameForNode(node.getName()));
		}
			...
			for (NodeParameter param : node.getParameterMappings().values()) {
				for (int i = 1; i <= multiplicity; i++) {
					if (!param.isStringValue()) {
						addParameterMapping(run, node, param, i);
					}
				}
			}
		}
	...
	public static Run initNodeInstanceRuntimeParameters(Run run, Node node, int nodeInstanceId)
		...
		run.createRuntimeParameter(node, nodeInstanceId, RuntimeParameter.NODE_NAME_KEY, node.getName(),
				RuntimeParameter.NODE_NAME_DESCRIPTION);

		run.createRuntimeParameter(node, nodeInstanceId, RuntimeParameter.NODE_ID_KEY, String.valueOf(nodeInstanceId),
				RuntimeParameter.NODE_ID_DESCRIPTION);

		run.createRuntimeParameter(node, nodeInstanceId, RuntimeParameter.CLOUD_SERVICE_NAME, cloudServiceName,
				RuntimeParameter.CLOUD_SERVICE_DESCRIPTION);

		ImageModule image = node.getImage();
		String imageId = image.extractBaseImageId(cloudServiceName);
		run.createRuntimeParameter(node, nodeInstanceId, RuntimeParameter.IMAGE_ID_PARAMETER_NAME, imageId,
				RuntimeParameter.IMAGE_ID_PARAMETER_DESCRIPTION);

		String imagePlatform = image.getPlatform();
		run.createRuntimeParameter(node, nodeInstanceId, RuntimeParameter.IMAGE_PLATFORM_PARAMETER_NAME, imagePlatform,
		...
		run.addNodeInstance(node, nodeInstanceId);
	...
	private static Run initNodeInstanceRuntimeParametersFromImageParameters(Run run, Node node, int nodeInstanceId)
		...
		for (Parameter param : image.getParameterList()) {
			...
			if (filter.contains(category) || cloudService.equals(category)) {
				...
				run.createRuntimeParameter(node, nodeInstanceId, param.getName(), initialValue, param.getDescription(),
						param.getType());
			}
		}
	...
	public static void addParameterMapping(Run run, Node node, NodeParameter param, int i) {
		...
		input.addMappedRuntimeParameterName(insertMultiplicityIndexInParameterName(node.getName()
				+ RuntimeParameter.NODE_PROPERTY_SEPARATOR + param.getName(), i));
		if (input.isSet()) {
	...
	public static String insertMultiplicityIndexInParameterName(String name, int index) {
		String[] parts = name.split(RuntimeParameter.NODE_PROPERTY_SEPARATOR);
		return parts[0] + RuntimeParameter.NODE_MULTIPLICITY_INDEX_SEPARATOR + index
				+ RuntimeParameter.NODE_PROPERTY_SEPARATOR + parts[1];
	}

	public static String extractInitialValue(Parameter parameter, Node node, Run run) {
	...
	private static String extractNodeParameterValue(Parameter parameter) {
		...
		String value = "";
		if (((NodeParameter)parameter).isStringValue()) {
	...
	public static Map<String, Node> getNodes(Run run) throws ValidationException {
		...
		if (module.getCategory() != ModuleCategory.Deployment) {
			throw new SlipStreamInternalException("getNodes can only be used with a Deployment module");
		}
	...
	private void checkParameterIsValid(Node node, Parameter parameter) throws ValidationException {
		...
		if (!node.getParameters().containsKey(paramName) && !node.getImage().getParameters().containsKey(paramName)
				&& !paramsToFilter.contains(paramName)) {
			throw new ValidationException("Unknown parameter: " + parameter.getName() + " in node: " + node.getName());
		}
	}
	...
	protected Map<String, ConnectorInstance> resolveCloudServices(Module module, User user,
		...
		Map<String, ConnectorInstance> cloudServiceNamesPerNode = new HashMap<String, ConnectorInstance>();
		...
		for (Node node : deployment.getNodes().values()) {
			...
			cloudServiceNamesPerNode.put(nodeName, new ConnectorInstance(cloudServiceName, null));
		}
	...
	protected void initExtraRunParameters(Module module, Run run) throws ValidationException {
		...
		for (Node node : deployment.getNodes().values()) {
			...
			insertNewRunParameterForNode(run, node, RuntimeParameter.MULTIPLICITY_PARAMETER_NAME,
					String.valueOf(multiplicity), RuntimeParameter.MULTIPLICITY_PARAMETER_DESCRIPTION);

			insertNewRunParameterForNode(run, node, RuntimeParameter.NODE_NAME_KEY,
					node.getName(), RuntimeParameter.NODE_NAME_DESCRIPTION);

			insertNewRunParameterForNode(run, node, RuntimeParameter.MAX_PROVISIONING_FAILURES,
					String.valueOf(node.getMaxProvisioningFailures()),
					RuntimeParameter.MAX_PROVISIONING_FAILURES_DESCRIPTION);

			insertNewRunParameterForNode(run, node, Run.NODE_INCREMENT_KEY, String.valueOf(multiplicity + 1),
					Run.NODE_INCREMENT_DESCRIPTION);

			String cloudService = run.getParameterValue(
					constructParamName(node.getName(), RuntimeParameter.CLOUD_SERVICE_NAME),
					CloudImageIdentifier.DEFAULT_CLOUD_SERVICE);
			boolean runBuildRecipes = node.getImage().hasToRunBuildRecipes(cloudService);
			insertNewRunParameterForNode(run, node, Run.NODE_RUN_BUILD_RECIPES_KEY,
					String.valueOf(runBuildRecipes), Run.NODE_RUN_BUILD_RECIPES_DESCRIPTION);
		}
	}
			...
			if (params != null) {
				for (Parameter parameter : params) {
					if (parameter.getName().equals(RuntimeParameter.MULTIPLICITY_PARAMETER_NAME)) {
						...
						key = constructNodeParamName(node, Run.NODE_INCREMENT_KEY);
	...
	protected void validateModuleForRun(Module module) throws ValidationException {
		...
		// Deployment module that is ready for run should define nodes on
		// itself.
	...
	private static void insertNewRunParameterForNode(Run run, Node node, String name, String value, String description,
		...
		Parameter rp = new Parameter(key, value, description);
		run.setParameter(rp);
	}
