...
public class ParametersFactory {
	...
	public static List<ExtraDisk> getExtraDisks(Map<String, Connector> connectors) {
	...
	public static Map<String, Parameter> getServiceConfigurationParametersTemplate(String[] connectorClassNames)
			throws ValidationException {
	...
	private static Map<String, Parameter> getServiceConfigurationParametersTemplate(Map<String, Connector> connectors)
			throws ValidationException {
	...
	public static User addParametersForEditing(User user) throws ValidationException, ConfigurationException {

		Map<String, UserParameter> execParameters = new ExecutionControlUserParametersFactory().getParameters();

		Map<String, Connector> connectors = ConnectorFactory.getConnectors();

		Map<String, UserParameter> templateParameters = ParametersFactory.getUserParametersTemplate(connectors);
		...
		for (Entry<String, UserParameter> template : templateParameters.entrySet()) {
			...
			Parameter existingParam = user.getParameter(templateParam.getName());
	...
	protected static void resetCloudServiceNameEnum(User user, Map<String, UserParameter> execParameters) {
		// Reset enum for cloud service name (in case connectors changed since)
		String cloudServiceNameKey = Parameter.constructKey(ParameterCategory.General.toString(),
				ExecutionControlUserParametersFactory.DEFAULT_CLOUD_SERVICE_PARAMETER_NAME);
		Parameter cloudServiceParameter = user.getParameter(cloudServiceNameKey);
		cloudServiceParameter.setEnumValues(execParameters.get(cloudServiceNameKey).getEnumValues());
	}

	private static Map<String, UserParameter> getUserParametersTemplate(Map<String, Connector> connectors)
			throws ValidationException {
	...
	public static Module addParametersForEditing(Module module) throws ValidationException, ConfigurationException {
	...
	private static void setParameters(Module module, Map<String, ModuleParameter> parameters)
			throws ValidationException {
